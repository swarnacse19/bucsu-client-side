        ...
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 p-6">
          <div className="flex items-center gap-4">
            ...
            <div className="text-white">
              ...
              <p className="text-indigo-100">CR Election Candidacy</p>
            </div>
          </div>
        </div>
                ...
                {election?.authorities?.length > 0 ? (
                  election.authorities.map((auth, idx) => (
                    <li key={idx} className="flex items-start gap-2">
                      <span className="text-indigo-500 mt-1">•</span> {auth}
                    </li>
                  ))
                ) : (
              ...
              <ul className="space-y-1 text-sm text-slate-600">
                {election?.positions?.length > 0 ? (
                  election.positions.map((pos, idx) => (
                    <li key={idx} className="flex items-start gap-2">
                      <span className="text-indigo-500 mt-1">•</span> {pos}
                    </li>
                  ))
                ) : (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="flex flex-col items-center mb-6">
                <div className="relative">
                  <div className="w-28 h-28 rounded-full overflow-hidden border-4 border-indigo-100 bg-gray-100">
                    {imagePreview ? (
                      <img
                        src={imagePreview}
                        alt="Preview"
                  ...
                  <label className="absolute bottom-0 right-0 w-10 h-10 bg-indigo-600 text-white rounded-full flex items-center justify-center cursor-pointer hover:bg-indigo-700 transition-colors">
                    <FaUpload />
                    <input
                      type="file"
                      accept="image/*"
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  rows={2}
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 resize-none"
